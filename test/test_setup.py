...
class TestSetup:
    def test_setup_load(self):
        _ = Setup.from_file(
            os.path.join(test_dir, "time_aware_setup_with_defaults.yaml")
        )

        with pytest.raises(SetupLoadError, match="Unknown setup schema version: 10"):
            Setup("version: 10")
        with pytest.raises(
            SetupLoadError, match='Setup does not define "setup" section'
        ):
            Setup("name: Broken setup")
        with pytest.raises(
            SetupLoadError, match="Default qubit parameters " "defined repeatedly .*"
        ):
            Setup(
                """
            setup:
        ...
        with pytest.raises(
            SetupLoadError,
            match=r"Parameters for qubit\(s\) " r"X defined repeatedly .*",
        ):
            Setup(
                """
            setup:
    ...
    def test_setup_get(self):
        ...
        with pytest.raises(
            KeyError,
            match=r'Parameter "param_without_default" '
        ...
        with pytest.raises(
            KeyError, match=r'Parameter "t1" is not defined for' r" qubit\(s\) Qother"
        ):
            Setup(
                """
            setup:
