...
from quantumsim.models import perfect_qutrits as lib3
...
class TestCompiler:
    ...
    def test_zz_parity_compilation(self):
        ...
        def leaky_cz(qubit1, qubit2, leakage_rate):
            def _cphase():
                generator = np.zeros((9, 9))
                generator[2, 4] = 1
                generator[4, 2] = 1
                angle_frac = 1 - np.arcsin(np.sqrt(leakage_rate)) / pi
                unitary = expm(-1j * pi * angle_frac * generator)
    ...
    def test_compilation_with_placeholders(self):
        ...
        zz_pc = optimize(zz_parametrized, bases_in, bases_out, qubits=[0, 1, 2])
        assert isinstance(zz_pc, Circuit)
        assert len(list(zz_pc.operations())) == 6
        ...
        zz_pc = optimize(zz_parametrized, bases_in, bases_out, qubits=[0, 1, 2])
        assert len(list(zz_pc.operations())) == 4
        zz_new = zz_parametrized(angle1=angle1, phase02=phase02, foo='bar')
        zz_pc = optimize(zz_new, bases_in, bases_out, qubits=[0, 1, 2])
        assert len(list(zz_pc.operations())) == 2
        assert zz_pc.finalize().ptm(bases_in, bases_out) == approx(ptm_ref)
