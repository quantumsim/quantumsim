...
class TestCphase:
    ...
    def test_trace_preserve(self):
        ...
        cphase(
            dm10_gpu, np.uint32(
                (1 << qbit1) | (
                    1 << qbit2)), np.uint32(no_qubits), block=(
                32, 32, 1), grid=(
                    32, 32, 1))
...
class TestHadamard():
    ...
    def test_trace_preserve(self):
        ...
        for qbit in range(no_qubits):
            ...
            hadamard(
                dm10_gpu, np.uint32(
                    1 << qbit), np.float64(0.5), np.uint32(no_qubits), block=(
                    32, 32, 1), grid=(
                    32, 32, 1))
...
class TestDMReduce:
    ...
    def test_trace_preserve(self):
        ...
        dm9_0 = np.zeros(
            (2**(no_qubits - 1), 2**(no_qubits - 1)), np.complex128)
        dm9_1 = np.zeros(
            (2**(no_qubits - 1), 2**(no_qubits - 1)), np.complex128)
...
class TestGetDiag:
    ...
    def test_get_diag(self):
        ...
        dm9 = np.random.random(
            (2**(no_qubits - 1), 2**(no_qubits - 1))).astype(np.complex128)
...
class TestDMInflate:
    ...
    def test_inverse_of_reduce(self):
        ...
        dm9_0 = np.random.random(
            (2**(no_qubits - 1), 2**(no_qubits - 1))).astype(np.complex128)
        dm9_0 += 1j * \
            np.random.random((2**(no_qubits - 1), 2**(no_qubits - 1)))
        dm9_0 = dm9_0 + dm9_0.T.conj()  # make hermitian
        dm9_0_gpu = drv.to_device(dm9_0)
        dm9_1 = np.random.random(
            (2**(no_qubits - 1), 2**(no_qubits - 1))).astype(np.complex128)
        dm9_1 += 1j * \
            np.random.random((2**(no_qubits - 1), 2**(no_qubits - 1)))
