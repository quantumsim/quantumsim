...
def compile_chain(chain, bases_in=None, bases_out=None,
                  *, optimize_bases=True, sv_cutoff=1e-5):
    graph = CircuitGraph(chain, bases_in=bases_in, bases_out=bases_out)
    compile_graph(graph, optimize_bases=optimize_bases,
                  sv_cutoff=sv_cutoff)
    return graph.to_operation()


def compile_graph(graph, *, optimize_bases=True, sv_cutoff=1e-5):
    stage_compile_all(graph, optimize_bases=optimize_bases,
                      sv_cutoff=sv_cutoff)
    stage_merge_operations(graph)


def stage_compile_all(graph, *, optimize_bases=True, sv_cutoff=1e-5):
    queue = CompilerQueue(graph.nodes)
    # DANGER: in the case of a bug in picking the right bases (nodes
    # continuously update basis of each other) the loop may never end.
    while len(queue) > 0:
        compile_next_in_queue(queue, optimize_bases=optimize_bases,
                              sv_cutoff=sv_cutoff)


def stage_merge_operations(graph):
    """
...
class CircuitGraph:
    ...
    def to_operation(self):
        if len(self.nodes) > 1:
            return self.nodes[0].op.from_sequence(
...
def optimal_bases(node, sv_cutoff=1e-5):
    ...
    ----------
    node : Node
        Node of a circuit graph.
    sv_cutoff : float
        The smallest possible singular value, taken into account.
    ...
    """
    d_in = np.prod([b.dim_pauli for b in node.bases_in_tuple])
    d_out = np.prod([b.dim_pauli for b in node.bases_out_tuple])
...
def compile_next_in_queue(queue, *, optimize_bases=True, sv_cutoff=1e-5):
    """
    ...
    b_out = node.bases_out_tuple
    node.op = node.op.set_bases(b_in, b_out)
    if optimize_bases:
        b_in, b_out = optimal_bases(node, sv_cutoff)
        node.op = node.op.set_bases(b_in, b_out)
